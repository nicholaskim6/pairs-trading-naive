...
data['Diff Accel'] = dataDiff

list2 = data['Diff'].tolist()
list3 = []
for i in range(1,506):
    list3.append(avgList(list2[:i]))
data['Historic Mean'] = list3


past20Diff = []
next20Diff = []
list4 = data['Diff Rate'].tolist()

for i in range(20):
    past20Diff.append(0)
for i in range(20,505):
    past20Diff.append(avgList(list4[i-20:i]))
data['past20DiffRate'] = past20Diff

for i in range(505-20):
    next20Diff.append(avgList(list4[i:i+20]))
for i in range(505-20,505):
    next20Diff.append(0)
data['next20DiffRate'] = next20Diff
...
# Creating trading signal based on past 20 day difference velocity
dataList = data['Diff'].tolist();    
tradeSignal = []
for i in range(505):
    peak = False
    currentDiff = data['Diff'][i]
    if(data['past20DiffRate'][i] > 0 and data['next20DiffRate'][i] < 0):
        peak = True
    if (peak == True):
...
data.drop(['past20DiffRate', 'next20DiffRate', 'Diff Rate', 'Diff Accel'], axis = 1).plot(title='Stock Time Series for Cluster %s' % str(pairs[0]),ylim = (-0.5,0.5))
...
for i in range(20,505-20):
    xtemp = []
    xtemp.append(data['Historic Mean'][i])
    xtemp.append(data['past20DiffRate'][i])
    xtemp.append(data['next20DiffRate'][i])
    xtemp.append(data['Diff Accel'][i])
...
for i in range(len(pairs)-testingInstances):
    ...
    for j in range(20,505-20):
        xtemp = []
        xtemp.append(data['Historic Mean'][j])
        xtemp.append(data['past20DiffRate'][j])
        xtemp.append(data['next20DiffRate'][j])
        if (data['Diff Accel'][j] < -100 or data['Diff Accel'][j] > 100):
            if (data['Diff Accel'][j] < -100):
                xtemp.append(-20)
            if (data['Diff Accel'][j] > 100):
                xtemp.append(20)
        else:
            xtemp.append(data['Diff Accel'][j])
        X_train.append(xtemp)
        y_train.append(data['Trade Signal'][j])
...
LR = tree.DecisionTreeClassifier() #This is the current model
...
for i in range(0,505):
    ...
    if (tradeOpen == True and appended == False):
        if (dayCounter > 80 or (data['past20DiffRate'][i] < 0 and data['next20DiffRate'][i] > 0)):
            tradeOpen = False
...
firstDiff = data[tickers[0]][150] - data[tickers[0]][0]
secondDiff = data[tickers[1]][150] - data[tickers[1]][0]
...
for i in range(len(pairs) - testingInstances, len(pairs)):
    ...
    for j in range(505):
        ...
        if (tradeOpen == True and appended == False):
            if (dayCounter > 80 or (data['past20DiffRate'][j] < 0 and data['next20DiffRate'][j] > 0)):
                tradeOpen = False
                tradePosition.append(2)
                appended = True
        if (tradeOpen == True):
...
# MODIFIED TEST AGAINST MANUAL TRADING RULE. Do not use for second strategy.
# testingInstances = 20
...
#     tradeReturns.append(tradeReturn)
#     firstDiff = data[tickers[0]][170] - data[tickers[0]][0]
#     secondDiff = data[tickers[1]][170] - data[tickers[1]][0]
#     holdReturn = 0.5*(1+firstDiff) + 0.5*(1+secondDiff)
#     holdReturns.append(holdReturn)
